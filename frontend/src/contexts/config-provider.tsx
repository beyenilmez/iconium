...
import { GetConfig, SetConfigField as SetConfigField_backend } from 'wailsjs/go/main/App';
...
export const ConfigProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    GetConfig().then((initialConfig: main.Config) => {
  ...
  const setConfigField = (key: keyof main.Config, value: any) => {
    if (config) {
      ...
      SetConfigField_backend(strKey, value)
        ...
        .catch(error => {
          console.error(`Failed to set config field ${key}:`, error);
        });
    }
  };

  return (
    <ConfigContext.Provider value={{ config, initialConfig, setConfig, setConfigField }}>
      {children}
